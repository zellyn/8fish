...
func TestUIByteBudget(t *testing.T) {
	...
	bounds := []struct{ label, what string }{
		...
		{"m8main", "cold start, main loop, book-to-aux copy"},
		{"copyx", "the big book: ProRWTS2 reader install, zp swap, load + verify + latch"},
		{"m8machine", "machine check (128K aux probe), whose-turn-is-it"},

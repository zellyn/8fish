...
func main() {
	...
	check(os.WriteFile("internal/rwts/gen.go", []byte(genGo(codeLen, len(blob), trackd1, sites)), 0o644))
	check(os.WriteFile("asm/rwts.inc", []byte(genInc(codeLen, len(blob), trackd1, sites)), 0o644))
...
func genGo(codeLen, blobLen, trackd1 int, sites []int) string {
	...
	fmt.Fprintf(&b, "\tBlobLen   = %d // code + denibble tables ($%04X-$%04X)\n", blobLen, reloc, reloc+blobLen-1)
...
func genInc(codeLen, blobLen, trackd1 int, sites []int) string {
	var b strings.Builder
	...
	b.WriteString("; addresses below and seeds RWTS_TRACKD1 from the loader's $41.\n\n")
	...
	fmt.Fprintf(&b, "RWTS_NSLOT    = %d      ; slot-poke operand addresses:\n", len(sites))
	b.WriteString(".macro RWTS_SLOT_LO_BYTES\n        .lobytes ")
	for i, s := range sites {
		if i > 0 {
			b.WriteString(", ")
		}
